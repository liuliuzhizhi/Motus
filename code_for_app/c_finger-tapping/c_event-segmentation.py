"""
============================================================
手指轻叩 Step 3 / 5 - 事件检测 (单视频)
============================================================
后端使用方式:
    from c_event-segmentation import (
...
输入 signal_df 列: frame, is_valid, distance_4_8 (Step 2 产物)
输出 events_df 列:
...
class FingerTappingConfig:
    ...
    plot_signal_title: str = "Finger Tapping"
...
def round2_low_peak_filter(accepted, seg_det, cfg):
    ...
    # width_med = float(np.median(widths)) if len(widths) > 0 else 1.0
...
def detect_finger_tapping_events(signal_df: pd.DataFrame,
    ...
    signal_df : pd.DataFrame
        Step 2 产物。需包含列: frame, is_valid, distance_4_8
    cfg : FingerTappingConfig or None
    ...
    if signal_df is None or len(signal_df) == 0:
        return pd.DataFrame()
    for c in ("frame", "is_valid", "distance_4_8"):
    ...
    distance = signal_df["distance_4_8"].values.astype(float)
...
if __name__ == "__main__":
    DEBUG_SIGNAL_CSV = r"F:\Chinese Excluded Files\Research\Clinical\develop\code\debug_for_app\a_finger-tapping\b_kinematic-signal\debug_distance_4_8.csv"
    DEBUG_OUTPUT_PLOT = r"F:\Chinese Excluded Files\Research\Clinical\develop\code\debug_for_app\a_finger-tapping\c_event-segmentation\debug_event.png"
    ...
    cfg = FingerTappingConfig(
        ...
        plot_signal_title="DEBUG finger-tapping",
    )
    print("[DEBUG] 开始检测")
